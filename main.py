...
from summarizers.news_summary import format_technical_summary, summarize_article
...
def send_telegram_message(message):
    if not BOT_TOKEN or not CHANNEL_ID:
        raise RuntimeError("TELEGRAM_BOT_TOKEN and TELEGRAM_CHANNEL_ID must be set in .env")

    url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
    response = requests.post(
        url,
        data={"chat_id": CHANNEL_ID, "text": message, "disable_web_page_preview": False},
        timeout=20,
    )
...
def _make_job_key(job):
    # Deliberately exclude derived metadata such as location/role/seniority.
    # Parser improvements must not make an old posting look like a new vacancy.
    raw = "|".join([
        job.get("source", ""),
        job.get("company", ""),
        job.get("title", ""),
...
def _requirements_text(job):
    ...
    if qualifications:
        qualification_text = "\n".join(f"  • {item}" for item in qualifications[:4])
    else:
...
def process_news():
    ...
    for article in articles:
        ...
        if not is_recent_article(article):
            print(f"[NEWS OLD] {article.get('published', '')} | {article['title']}")
            continue

        article["ic_score"] = score
        article["keywords"] = keywords
        accepted.append(article)
        print(f"[NEWS ACCEPT] {score:2d} | {article['title']} | {keywords}")
    ...
    if BASELINE_ONLY:
        ...
        print(f"News baseline finished. Recorded {baseline_count} existing articles; sent 0.")
        return
    ...
    for article in accepted:
        ...
        summary_text = format_technical_summary(summarize_article(article))
        company = article.get("company")
        company_line = f"🏢 Company: {company}\n" if company else ""
        published_line = f"🗓 Published: {article['published']}\n" if article.get("published") else ""
        message = (
            "📰 IC TECHNOLOGY NEWS\n\n"
            f"{article['title']}\n\n"
            f"{company_line}"
...
def process_jobs():
    ...
    for raw_job in jobs:
        if raw_job.get("detail_api_url") and raw_job.get("location") and not is_vietnam_job(raw_job):
            print(
                f"[JOB REJECT LOCATION] | {raw_job.get('company', '')} | "
                f"{raw_job.get('title', '')} | {raw_job.get('location', '')}"
            )
            continue

        pre_related, _, pre_score, _ = classify_job(raw_job, threshold=5, require_vietnam=False)
        if not pre_related:
            print(f"[JOB REJECT] {pre_score:2d} | {raw_job.get('company', '')} | {raw_job.get('title', '')}")
            continue
        ...
        if related:
            ...
            print(
                f"[JOB REJECT] {score:2d} | {job.get('company', '')} | "
                f"{job.get('title', '')} | {job.get('location', '')}"
            )

    accepted.sort(key=lambda item: (-item.get("job_score", 0), item.get("company", ""), item.get("title", "")))
    print(f"\nTarget Vietnam IC jobs: {len(accepted)}/{len(jobs)}")

    if BASELINE_ONLY:
        baseline_count = 0
        for job in accepted:
            if job_exists(job["job_key"], job):
                continue
            save_job(job)
            baseline_count += 1
            print(f"[JOB BASELINE] {job['company']} | {job['title']}")
        print(f"Jobs baseline finished. Recorded {baseline_count} existing jobs; sent 0.")
        return
